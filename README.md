# Hi, I'm Munir 👋

<p align="center">
  <b>Founder of <a href="https://orquio.com">Orquio</a> · Building AI products in public</b><br/>
  <a href="https://munito.dev">munito.dev</a> ·
  <a href="https://www.linkedin.com/in/munir-torres">LinkedIn</a> ·
  <a href="https://x.com/MunitoDev">@MunitoDev</a> ·
  <a href="mailto:torres@orquio.com">torres@orquio.com</a>
</p>

19, from Madrid. I build desktop and web products with AI agents, mostly for Spanish-speaking
users, and I share the process as it happens.

### Products

- **[Orquio](https://orquio.com)** · SaaS that brings local businesses online
- **[Layco](https://github.com/Mun1to/Layco)** · local-first AI co-worker floating on your screen
- **[VoCript](https://github.com/Mun1to/VoCript)** · voice and system audio into text, 100% offline
- **[vidorq](https://github.com/Mun1to/vidorq)** · AI video editor built on DaVinci Resolve Free
- **Adequio** · my agentic dev environment: terminals, sessions and agent teams

### Apps and tools

- **[Expoal](https://github.com/Mun1to/Expoal)** · save any video to your disk, no third-party sites
- **[Moneorq](https://github.com/Mun1to/Moneorq)** · your money, made clear
- **[froede](https://github.com/Mun1to/froede)** · click your localhost page, edit the real source
- **[TaskbarGroupsFluent](https://github.com/Mun1to/TaskbarGroupsFluent)** · taskbar shortcut groups, WPF and .NET 8
- **[hotkeyconfig](https://github.com/Mun1to/hotkeyconfig)** · program ch57x macropads and bind anything
- **[Klipse](https://github.com/Mun1to/Klipse)** · lock the mouse cursor to one monitor
- **[LengHist](https://github.com/Mun1to/LengHist)** · interactive guide to 100 programming languages

### Stack

<p>
  <img src="https://skillicons.dev/icons?i=html,css,tailwind,js,ts,react,vite,tauri" alt="frontend" /><br/>
  <img src="https://skillicons.dev/icons?i=rust,python,fastapi,nodejs,cs,dotnet,supabase,cloudflare" alt="backend" /><br/>
  <img src="https://skillicons.dev/icons?i=git,github,vscode,obsidian" alt="tools" />
</p>

<p>
  <img src="https://img.shields.io/badge/Claude%20Code-D97757?logo=claude&logoColor=white" alt="Claude Code" />
  <img src="https://img.shields.io/badge/Ollama-000000?logo=ollama&logoColor=white" alt="Ollama" />
  <img src="https://img.shields.io/badge/DaVinci%20Resolve-233A51?logo=davinciresolve&logoColor=white" alt="DaVinci Resolve" />
  <img src="https://img.shields.io/badge/Antigravity-1F1F1F?logoColor=white" alt="Antigravity" />
</p>

<details>
<summary>🇪🇸 Leer en español</summary>

<br/>

19 años, de Madrid. Construyo productos de escritorio y web con agentes de IA, sobre todo
para el mundo hispanohablante, y enseño el proceso mientras ocurre.

**Productos**

- **[Orquio](https://orquio.com)** · SaaS para digitalizar el comercio local
- **[Layco](https://github.com/Mun1to/Layco)** · copiloto de IA local que flota en tu pantalla
- **[VoCript](https://github.com/Mun1to/VoCript)** · tu voz y el audio del PC a texto, 100% sin internet
- **[vidorq](https://github.com/Mun1to/vidorq)** · editor de vídeo con IA sobre DaVinci Resolve Free
- **Adequio** · mi entorno de desarrollo con agentes: terminales, sesiones y equipos

**Apps y herramientas**

- **[Expoal](https://github.com/Mun1to/Expoal)** · guarda cualquier vídeo en tu disco, sin webs de terceros
- **[Moneorq](https://github.com/Mun1to/Moneorq)** · tu dinero, claro de un vistazo
- **[froede](https://github.com/Mun1to/froede)** · haz clic en tu web de localhost y edita el código real
- **[TaskbarGroupsFluent](https://github.com/Mun1to/TaskbarGroupsFluent)** · grupos de accesos en la barra de tareas
- **[hotkeyconfig](https://github.com/Mun1to/hotkeyconfig)** · programa macropads ch57x y asigna lo que quieras
- **[Klipse](https://github.com/Mun1to/Klipse)** · fija el cursor del ratón a un solo monitor
- **[LengHist](https://github.com/Mun1to/LengHist)** · guía interactiva de 100 lenguajes de programación

</details>

<sub><i>Change the plan but never the goal.</i></sub>
